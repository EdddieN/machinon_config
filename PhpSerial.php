...
class PhpSerial
{
     ...
     * @return bool
     */
    public function deviceOpen($mode = "r+b")  //, $raw=true, $echoOn=false)
    {
        if ($this->_dState === SERIAL_DEVICE_OPENED) {
		...
		/*
		if ($this->_os === "linux") {
			// set echo and line ending options
			//$this->_exec("stty -F " . this->_device . (($raw == true) ? " -raw" : "") . (($echoOn == true) ? " echo " : " -echo -echoe -echok -echonl -echoprt -echoctl"));
			//$this->_exec("stty -F " . $this->_device . " -nl -echo -echoe -echok -echonl -echoprt -echoctl");
		}
		*/
